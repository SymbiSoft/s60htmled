...
class HTMLEditor:
    ...
    def __init__(self):
        self.editor = appuifw2.Text(edit_callback=self.changeEvent)
...
#        print repr(txt)
        ...
        if i >= 0:
            self.editor.set_pos(self.editor.get_pos() + i + len(find_text))
            return True
    ...
    def doReplace(self, txt):
        ...
        ed.delete(ed.get_pos()-len(self.find_text), len(self.find_text))
        ed.add(txt)
    
    def findText(self):
        ans = appuifw2.query(u("Find"), "text")
        if ans is None: return
        self.find_text = ans
        self.doFind(self.find_text)
    ...
    def replaceText(self):
        ...
        if ans is None: return
        if self.doFind(ans[0]):
            self.doReplace(ans[1])
        
    def findEOL(self):
        self.doFind(u"\u2029")

    def run(self):
        self.editor.bind(key_codes.EKeyYes, self.gkeyPressed)
        ...
        appuifw2.app.exit_key_text = u("Insert tag")
        self.app_lock.wait()
